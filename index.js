...
( async function() {
	if (!process.env.mongoURL) return console.warn('MongoDB URL is not provided in the environment variables, skipping database connection...');
...
async function InteractionHandler(interaction, type) {
    ...
    try {
        ...
        if (component.admin) {
            ...
            if (interaction.user.id !== process.env.OWNER_ID) return await interaction.reply({ content: `⚠️ Only bot owners can use this command!`, flags: 64 });
        }
